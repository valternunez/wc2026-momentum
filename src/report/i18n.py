...
STRINGS = {
    "en": {
        ...
        "BOTTOM_TEXT": "On the surface the momentum killer is real. The same teams sag without a break too, so regression to the mean is most of it — but not all: the break still drops the leader about {{GAP}} points further than no break does — {{GAP_CLAUSE}}. Whether that gap firms up or fades, the knockouts will tell. And there's a second question the heat data raises: whether a break in every match was needed at all. The verdict stays open until the final.",
...
<p><a class="src" href="{{PAGES_URL}}">github.com/valternunez/wc2026-momentum ↗</a></p>''',
    ...
    "es": {
        ...
        "BOTTOM_TEXT": "En la superficie, pareciera que la muerte del momentum es real. Los mismos equipos se desinflan sin pausa también, así que la regresión a la media es la mayor parte — pero no todo: la pausa todavía baja al líder unos {{GAP}} puntos más que no tener pausa — {{GAP_CLAUSE}}. Si esa brecha se afirma o se desvanece, lo dirán las eliminatorias. Y hay una segunda pregunta que dejan los datos del calor: si hacía falta una pausa en cada partido siquiera. El veredicto sigue abierto hasta la final.",
