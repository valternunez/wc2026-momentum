...
STRINGS = {
    "en": {
        ...
        "S05_CAVEAT_BOX": '''Same statistic, same scale. The gold-standard control is the same 2026 teams at quiet minutes (−{{P26_DELTA}}), and Euro 2024, World Cup 2022 and the same-host Gold Cup 2025 land right with it (−{{EURO_DELTA}}, −{{WC22_DELTA}}, −{{GOLD_DELTA}}): national-team football regresses about −{{NOBREAK_LO}} to −{{NOBREAK_HI}} on its own. Clubs swing more (the Club World Cup drops −{{CWC_DELTA}}), which is why an earlier club-only comparison flattered the "same drop" read. The break (−{{HERO_DELTA}}) sits about {{GAP}} points below the same-teams control once their pre-break level is matched, {{GAP_CLAUSE}}. The sample is small, so it's suggestive, not proven. <span style="color:#5A5547">(An event-xT cross-check on 2022 agrees the slide is real.)</span>''',
