...
FRAG = {
    "en": {
        ...
        "tip_copa": "Copa América 2024: national teams in the same US summer, but the smallest sample here (61 on-top windows), so its low −10 is noisy. Read it next to the others, not on its own.",
        ...
        "exp_lost": '''At the <span class="ev-min">{m}'</span> hydration break {leader} were on top, then lost <span class="ev-mom">{x}</span> momentum over the next five minutes.''',
        "exp_pushed": '''At the <span class="ev-min">{m}'</span> hydration break {leader} were on top and pushed <span class="ev-mom">{x}</span> further ahead.''',
    ...
    "es": {
        ...
        "tip_copa": "Copa América 2024: selecciones en el mismo verano de EE. UU., pero la muestra más chica de aquí (61 ventanas con un equipo dominando), así que su −10 bajo es ruidoso. Léelo junto a los demás, no por sí solo.",
        ...
        "exp_lost": '''En la pausa de hidratación del minuto <span class="ev-min">{m}'</span>, {leader} dominaba y luego perdió <span class="ev-mom">{x}</span> de momentum en los cinco minutos siguientes.''',
        "exp_pushed": '''En la pausa de hidratación del minuto <span class="ev-min">{m}'</span>, {leader} dominaba y sacó <span class="ev-mom">{x}</span> más de ventaja.''',
...
STRINGS = {
    "en": {
        ...
        "S02_LEAD": "Take the team on top before a break and track what it loses over the next five minutes, split by what kind of stoppage cut in. Every bar leans the same way, but the hydration break leans furthest.",
        ...
        "S04_DURATION": '''We can finally put a clock on it. ESPN logs when each stoppage starts and ends, so the hydration breaks come in at a median of <strong style="font-weight:600;color:#EFEBDF">{{DUR_MEDIAN}}</strong> (from {{DUR_MIN}} to {{DUR_MAX}}) — two to three times longer than a typical injury stoppage. And the longer the break runs, the harder the leader falls: short breaks cost about −{{DUR_SHORT}}, the long ones about −{{DUR_LONG}}, roughly {{DUR_SLOPE}} more points lost for every extra minute on the touchline. Break length isn't handed out at random, so read it as a clue rather than proof — but it points the same way as the huddle, not the water.''',
        "S04_CONCL": '''If a water break were only about <em style="font-style:italic">rest</em>, a long VAR pause should match it. So far it doesn't quite, which hints at the <strong style="font-weight:600;color:#EFEBDF">coaching window</strong> a break creates. Belgium's Rudi Garcia put it plainly: <span style="color:#EFEBDF">"for me, it's a coaching break more than a cooling break."</span> <a class="src" href="https://www.aljazeera.com/sports/2026/6/20/hydration-break-boos-how-fifa-united-players-fans-coaches-at-world-cup">[Al Jazeera]</a> One caveat we won't bury: the comparison is mostly by stoppage type rather than perfectly duration-matched (we now time the breaks themselves, but VAR and injury timings are patchier), and the "with sub" injury split is confounded by the substitution itself. Once we control for pre-break momentum, the hydration and VAR intervals <strong style="font-weight:600;color:#EFEBDF">overlap</strong>. Suggestive, not a verdict. And a VAR pause still lets a coach shout instructions from the touchline, so if anything this comparison understates the coaching window rather than inventing it.''',
...
<li><strong>Break durations, now partly measured.</strong> ESPN's start/end-delay timestamps give an exact length for {{DUR_N}} of {{DUR_N_ALL}} hydration breaks (median {{DUR_MEDIAN}}); VAR and injury coverage is thinner, so the type comparison is only partly duration-matched. Longer breaks track bigger drops, but break length isn't randomly assigned — a clue, not an instrument.</li>
...
<p><a class="src" href="{{PAGES_URL}}">github.com/valternunez/wc2026-momentum ↗</a></p>''',
    ...
    "es": {
        ...
        "S01_FOLLOW": '''La queja de Van Dijk es sobre el ritmo y los cortes comerciales. La acusación también es táctica, y en la cabina incluso tienen un nombre para ello. Emma Hayes, de ITV, las llama <em style="font-style:italic">pausas de momentum</em>: <span style="color:#1A1813">«ventajosas para el equipo que está perdiendo el momentum… cuando dominas, no las quieres; cuando vas cayendo, sí.»</span> <a class="src" href="https://www.espn.com/soccer/story/_/id/48945011/why-there-drinks-breaks-2026-world-cup-fifa-criticised">[ESPN]</a> Un reinicio gratis para el equipo bajo presión. Y es esta misma idea la que los datos de este proyecto ponen a prueba.''',
        ...
        "S02_MOMTECH": '''Es el propio modelo de <a class="src" href="https://theanalyst.com/articles/what-is-match-momentum">amenaza esperada</a> de FotMob (en su payload lo etiquetan como «xT»), no el momentum oficial de Opta / Stats&nbsp;Perform que se ve en las transmisiones de la FIFA (es un modelo relacionado pero distinto). Se usa el de FotMob de la misma forma en todos los torneos aquí, así que la elección de modelo se cancela en la comparación 2026-vs-sin-pausa. Se reconstruye la curva solo con remates: con el mejor ajuste de decaimiento explica alrededor del 20% de la varianza (r&nbsp;&asymp;&nbsp;0.46). El resto es el juego de elaboración que premia el xT, una métrica real de amenaza, no ruido.''',
        "S02_LEAD": "Toma al equipo que dominaba antes de una pausa y mira qué pierde en los cinco minutos siguientes, según el tipo de interrupción que lo cortó. Todas las barras se inclinan igual pero la de hidratación es la que más lo hace.",
        ...
        "S04_DURATION": '''Por fin le podemos poner cronómetro. ESPN registra cuándo empieza y termina cada interrupción, así que las pausas de hidratación salen con una mediana de <strong style="font-weight:600;color:#EFEBDF">{{DUR_MEDIAN}}</strong> (de {{DUR_MIN}} a {{DUR_MAX}}) — dos o tres veces más largas que una pausa por lesión típica. Y entre más larga la pausa, más fuerte cae el que iba arriba: las cortas cuestan alrededor de −{{DUR_SHORT}}, las largas cerca de −{{DUR_LONG}}, unos {{DUR_SLOPE}} puntos más perdidos por cada minuto extra en la banda. La duración no se reparte al azar, así que tómalo como pista, no como prueba — pero apunta a lo mismo que el corrillo, no el agua.''',
        "S04_CONCL": '''Si una pausa de agua fuera solo cuestión de <em style="font-style:italic">descanso</em>, una pausa del VAR igual de larga debería igualarla. Por ahora no lo hace del todo, lo que apunta a la <strong style="font-weight:600;color:#EFEBDF">ventana de indicaciones técnicas</strong> que crea una pausa. El técnico de Bélgica, Rudi García, lo dijo: <span style="color:#EFEBDF">«para mí, es más una pausa de instrucción que una pausa de enfriamiento.»</span> <a class="src" href="https://www.aljazeera.com/sports/2026/6/20/hydration-break-boos-how-fifa-united-players-fans-coaches-at-world-cup">[Al Jazeera]</a> Un matiz que no escondemos: comparamos sobre todo por tipo de interrupción más que de forma perfectamente emparejada por duración (ya cronometramos las pausas, pero los tiempos de VAR y lesión son más irregulares), y la división de lesión «con cambio» está confundida por el propio cambio. Al controlar por el momentum previo, los intervalos de la hidratación y del VAR <strong style="font-weight:600;color:#EFEBDF">se solapan</strong>. Sugerente, no un veredicto. Y en una pausa del VAR el técnico igual puede gritar indicaciones desde la banda, así que esta comparación, si acaso, subestima la ventana de indicaciones técnicas en lugar de inventarla.''',
        ...
        "FOOT_ID_T": "Contraste por tipo entre pausas de hidratación, VAR y lesión. Las pausas se detectan a partir del relato de ESPN (no de un reloj fijo), y ahora cronometramos su duración con las marcas de inicio/fin de ESPN (la hidratación bien cubierta; VAR y lesión más irregulares).",
...
<li><strong>Duraciones de pausa, ahora en parte medidas.</strong> Las marcas de inicio/fin de ESPN dan una duración exacta para {{DUR_N}} de {{DUR_N_ALL}} pausas de hidratación (mediana {{DUR_MEDIAN}}); la cobertura de VAR y lesión es más delgada, así que la comparación por tipo solo está en parte emparejada por duración. Las pausas más largas van con caídas mayores, pero la duración no se asigna al azar — una pista, no un instrumento.</li>
