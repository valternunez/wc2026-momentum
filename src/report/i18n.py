...
<p><a class="src" href="{{PAGES_URL}}">github.com/valternunez/wc2026-momentum ↗</a></p>''',
    ...
    "es": {
        ...
        "S04_DURATION": '''ESPN cronometra cada pausa, del silbatazo a la reanudación: una mediana de <strong style="font-weight:600;color:#EFEBDF">{{DUR_MEDIAN}}</strong> (de {{DUR_MIN}} a {{DUR_MAX}}), dos o tres veces más larga que una pausa por lesión típica, y notablemente constante. ¿Una pausa <em style="font-style:italic">más larga</em> golpea más fuerte al que va arriba? Aquí los datos no alcanzan a decirlo: las pausas largas lo enfrían un poco más que las cortas (cerca de −{{DUR_LONG}} frente a −{{DUR_SHORT}}), pero esa diferencia es demasiado chica para distinguirla del ruido. Así que lo que parece pesar es el <strong style="font-weight:600;color:#EFEBDF">tamaño de la ventana</strong>, no las pequeñas diferencias en cuánto dura.''',
