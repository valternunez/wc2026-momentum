 ...
 *{margin:0;padding:0;box-sizing:border-box}
 ...
 .nums{display:flex;align-items:flex-end;gap:46px;margin-top:10px}
...
def build_share_card(out_path: str | Path | None = None) -> str:
    """Render the 1200x630 share card + 180px touch icon (needs Playwright). Returns the og path."""
    out = Path(out_path or OUT)
    out.parent.mkdir(parents=True, exist_ok=True)
    hyd, cwc = _finding_numbers()
    html = _CARD.replace("{HYD}", hyd).replace("{CWC}", cwc)
